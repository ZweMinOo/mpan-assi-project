    ...
    <script type="text/javascript">
	  ...
	  $.ajax({
			...
			success: function(GDPJson){
				console.log(GDPJson);
				GDP_data = GDPJson[1];
				chart_data=[['Year','Myammar sq.km']];
				for(var GDP of GDP_data)
				{ 
					if(GDP.date=='2010')
					{
						chart_data[1]=['2010',Number((GDP.value/10).toFixed(2))]
					}
					else if(GDP.date=='2011')
					{
						chart_data[2]=['2011',Number((GDP.value/10).toFixed(2))]
					}
					else if(GDP.date=='2012')
					{
						chart_data[3]=['2012',Number((GDP.value/10).toFixed(2))]
					}
					else if(GDP.date=='2013')
					{
						chart_data[4]=['2013',Number((GDP.value/10).toFixed(2))]
					}
					else if(GDP.date=='2014')
					{
						chart_data[5]=['2014',Number((GDP.value/10).toFixed(2))]
					}	
				}
